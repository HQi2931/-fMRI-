...
`chat`、`context`、`memory`、`literature` 和 `retrieval` 已承载科研问答：有限历史上下文、滚动摘要、显式确认的长期记忆、PDF 摄取、手动索引和可追溯引用。`core`、`multi_agent` 和通用 `plugins` 仍保留为扩展边界。当前结构以根目录 `AGENTS.md`、系统设计、fMRI Skill 设计和 ADR 为准。
